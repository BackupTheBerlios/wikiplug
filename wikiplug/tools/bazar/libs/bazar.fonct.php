			...
			if ($row == 1) {
				...
				$num = count($data);
					...
					if ($c==0) {
						...
						$output .= '<strong>'.$tab_labels[$c].'</strong>'.' : '.utf8_decode(str_replace('Ã¢ÂÂ','\'',$data[$c])) . "<br />\n";
					}

					//si une valeur est prÃ©sente, on l'affiche et on la rajoute au tableau des valeurs Ã  importer pour la fiche
			            elseif ($data[$c]!='') {
...
/**
 *
 * interface de choix des fiches Ã  exporter
 */
...
function baz_requete_bazar_fiche($valeur) {
	//on enleve les champs hidden pas necessaires Ã  la fiche
	...
	//pour une insertion d'une nouvelle fiche, on gÃ©nÃ©re l'id de la fiche
...
function baz_insertion_fiche($valeur) {
	//on teste au moins l'existence du titre car sans titre ca peut bugguer sÃ©rieusement
...
function baz_formulaire_des_listes($mode, $valeursliste = '') {
	...
	$html_valeurs_listes .= '</ul><a href="#" class="ajout_label_liste" title="'.BAZ_AJOUTER_LABEL_LISTE.'">'.BAZ_AJOUTER_LABEL_LISTE.'</a>'."\n".
							'</div>'."\n".
							'<div class="clear"></div>'."\n";
	//on rajoute une variable globale pour mettre le javascript en plus Ã  la fin
	$GLOBALS['js'] = ((isset($GLOBALS['js'])) ? $GLOBALS['js'] : '').'<script type="text/javascript" src="tools/bazar/libs/jquery-ui-1.8.6.custom.min.js"></script>
							<script type="text/javascript">
							  $(document).ready(function() {
...
function baz_gestion_listes() {
	...
	//il y a des donnees pour modifier une liste
		...
		/* ----------------- TODO: gÃ©rer les suppressions de valeurs dans les fiches associÃ©es pour garantir l'intÃ©gritÃ© des donnÃ©es
		//on vÃ©rifie si les valeurs des listes ont changÃ©es afin de garder de l'intÃ©gritÃ© de la base des fiches
		foreach ($_POST["ancienlabel"] as $key => $value) {
			//si la valeur de la liste a Ã©tÃ© changÃ©e, on rÃ©percute les changements pour les fiches contenant cette valeur
			if ( isset($_POST["label"][$key]) && $value != $_POST["label"][$key] ) {
				//TODO: fonction baz_modifier_metas_liste($_POST['NomWiki'], $value, $_POST['label'][$key]);
			}
		}

		//on supprime les valeurs des listes supprimÃ©es des fiches possÃ©dants ces valeurs
...
function encoder_en_utf8($txt) {
	// Nous remplacons l'apostrophe de type RIGHT SINGLE QUOTATION MARK et les & isolÃ©s qui n'auraient pas Ã©tÃ©
	// remplacÃ©s par une entitÃ© HTML.
	$cp1252_map = array(
