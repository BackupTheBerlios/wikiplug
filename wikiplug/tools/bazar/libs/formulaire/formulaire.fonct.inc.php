...
// +------------------------------------------------------------------------------------------------------+
// CVS : $Id: formulaire.fonct.inc.php,v 1.26 2011/03/22 09:33:23 mrflos Exp $
/**
* Formulaire
*
...
*@version       $Revision: 1.26 $ $Date: 2011/03/22 09:33:23 $
// +------------------------------------------------------------------------------------------------------+
*/

//comptatibilitÃ© avec PHP4...
...
function liste(&$formtemplate, $tableau_template, $mode, $valeurs_fiche)
{
	...
	{
		//on affiche la liste sous forme de liste dÃ©roulante
...
function checkbox(&$formtemplate, $tableau_template, $mode, $valeurs_fiche)
{
	if ($mode == 'saisie')
	{
		...
		foreach ($choixcheckbox as $id => $label) {
			...
			//teste si la valeur de la liste doit Ãªtre cochÃ©e par dÃ©faut
			if (in_array($id,$tab)) {
...
function tags(&$formtemplate, $tableau_template, $mode, $valeurs_fiche)
{
	...
	{
		$tags_javascript = '';
		//gestion des mots clÃ©s dÃ©ja entrÃ©s
		if (isset($valeurs_fiche[$tableau_template[1]])) {
	...
	elseif ( $mode == 'requete' ) {
		...
		//on copie tout de meme dans les metadonnÃ©es
...
function utilisateur_wikini(&$formtemplate, $tableau_template, $mode, $valeurs_fiche)
{
	...
	elseif ( $mode == 'requete' )
	{
		if (!isset($valeurs_fiche['nomwiki']))
		{
			...
			$messagemail = "Bonjour!\n\nVotre inscription sur le site a Ã©tÃ© finalisÃ©e, dorÃ©navant vous pouvez vous identifier avec les informations suivantes :\n\nVotre identifiant NomWiki : ".$nomwiki."\nVotre mot de passe : ". $valeurs_fiche['mot_de_passe_wikini'] . "\n\nA trÃ¨s bientÃ´t !\n\nSylvie Vernet, webmestre";
			...
			$lien = str_replace("/wakka.php?wiki=","",$GLOBALS['wiki']->config["base_url"]);
			$objetmail = '['.str_replace("http://","",$lien).'] Vos nouveaux identifiants sur le site '.$GLOBALS['wiki']->config["wakka_name"];
			$messagemail = "Bonjour!\n\nVotre inscription sur le site a Ã©tÃ© modifiÃ©e, dorÃ©navant vous pouvez vous identifier avec les informations suivantes :\n\nVotre identifiant NomWiki : ".$nomwiki."\nVotre mot de passe : ". $valeurs_fiche['mot_de_passe_wikini'] . "\n\nA trÃ¨s bientÃ´t !\n\nSylvie Vernet, webmestre";
			$headers =   'From: '.BAZ_ADRESSE_MAIL_ADMIN . "\r\n" .
			     'Reply-To: '.BAZ_ADRESSE_MAIL_ADMIN . "\r\n" .
			     'X-Mailer: PHP/' . phpversion();
			mail($valeurs_fiche['bf_mail'], remove_accents($objetmail), $messagemail, $headers);
...
function champs_mail(&$formtemplate, $tableau_template, $mode, $valeurs_fiche)
{
	if ( $mode == 'saisie' )
	{
		...
		//gestion des valeurs par dÃ©faut : d'abord on regarde s'il y a une valeur Ã  modifier,
		//puis s'il y a une variable passÃ©e en GET,
		//enfin on prend la valeur par dÃ©faut du formulaire sinon
		if (isset($valeurs_fiche[$tableau_template[1]])) {
...
/** mot_de_passe() - Ajoute un Ã©lÃ©ment de type mot de passe au formulaire
*
* @param    mixed   L'objet QuickForm du formulaire
...
/** fichier() - Ajoute un Ã©lÃ©ment de type fichier au formulaire
*
* @param    mixed   L'objet QuickForm du formulaire
...
/** image() - Ajoute un Ã©lÃ©ment de type image au formulaire
*
* @param    mixed   L'objet QuickForm du formulaire
* @param    mixed   Le tableau des valeurs des diffÃ©rentes option pour l'Ã©lÃ©ment image
* @param    string  Type d'action pour le formulaire : saisie, modification, vue,... saisie par dÃ©faut
* @return   void
*/
function image(&$formtemplate, $tableau_template, $mode, $valeurs_fiche) {
	...
	if ( $mode == 'saisie') {
		//on vÃ©rifie qu'il ne faut supprimer l'image
		if (isset($_GET['suppr_image']) && $valeurs_fiche[$type.$identifiant]==$_GET['suppr_image']) {
			...
			//on affiche les infos sur l'effacement du fichier, et on rÃ©initialise la variable pour le fichier pour faire apparaitre le formulaire d'ajout par la suite
		...
		if (isset($valeurs_fiche[$type.$identifiant]) && $valeurs_fiche[$type.$identifiant] != '') {			
			...
			//il y a bien le fichier image, on affiche l'image, avec possibilitÃ© de la supprimer ou de la modifier
			if (file_exists(BAZ_CHEMIN_UPLOAD.$valeurs_fiche[$type.$identifiant])) {
			...
			//le fichier image n'existe pas, du coup on efface l'entrÃ©e dans la base de donnÃ©es
			else {
		...
		//cas ou il n'y a pas d'image dans la base de donnÃ©es, on affiche le formulaire d'envoi d'image
		else {
			$formtemplate->addElement('file', $type.$identifiant, $label) ;
			//gestion du champs obligatoire
			if (($apparait_recherche==0) && isset($obligatoire) && ($obligatoire==1)) {
			...
			//TODO: la vÃ©rification du type de fichier ne marche pas
	...
	elseif ( $mode == 'requete' ) {
		if (isset($_FILES[$type.$identifiant]['name']) && $_FILES[$type.$identifiant]['name']!='') {
			//dans le cas d'une modification, on vÃ©rifie l'existance d'une image prÃ©cÃ©dente, que l'on supprime et remplace
...
* Revision 1.26  2011/03/22 09:33:23  mrflos
* version de travail
*
